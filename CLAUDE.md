...
### 2. Voice Pattern (REQUIRED) - Updated Dec 2024

**CRITICAL: DO NOT use "I don't know if this is you" or similar hedging phrases. Jesse HATES this.**

Use confident vulnerability - be direct about THEM, not uncertain about yourself:

```html
<!-- GOOD: Direct statement about them -->
...
<!-- BAD: Hedging/uncertain about yourself - NEVER USE -->
...
- "I don't have all the answers. But I can..." (vulnerability about WHAT you admit, not HOW you say it)
...
| Phrase | Why It Fails |
|--------|--------------|
| "I don't know if this is you, but..." | Uncertain, hedging |
...
| "No homework. No worksheets." | CRPO violation - disparages other approaches |
...
**Key patterns:**
- Direct statements about THEM (not disclaimers about yourself)
- "Sound familiar?" as the ONLY approved check-in
- "I don't have all the answers. But I can..." (confident vulnerability)
- Signature phrases: "WITH the client, not ahead"
- Process over outcomes
- Confident but not arrogant

**NEVER USE:** "I don't know if this is you", "Honestly...", "Right?", "Does that make sense?"
...
## pSEO Auto-Pass Quality Checklist (Dec 2024)

**CRITICAL: Every new pSEO page MUST pass ALL these checks before deployment. Follow this exactly and pages will auto-pass audits.**
...
| Citations | Include stats from CAMH, APA, CMHA, MHCC with source attribution |
...
**Stats Section Template (copy exactly):**
...
```javascript
// Check with: Array.from(document.querySelectorAll('script[type="application/ld+json"]')).map(s => JSON.parse(s.textContent)['@type'])
